"""
This code is based on the LongVideoBench dataset: https://github.com/longvideobench/LongVideoBench/blob/main/longvideobench/longvideobench_dataset.py
We appreciate the authors for providing the dataset and the code.
"""
...
def insert_subtitles(subtitles):
    return [subtitle.get("text", subtitle.get("line")) for subtitle in subtitles]

        
def insert_subtitles_into_frames(frames, frame_timestamps, subtitles, 
                                 starting_timestamp_for_subtitles, duration):
    interleaved_list, cur_i = [], 0
    
    for subtitle in subtitles:
        if "timestamp" in subtitle:
            start, end = subtitle["timestamp"]
            end = end if isinstance(end, float) else duration
        else:
            start, end = timestamp_to_seconds(subtitle["start"]), timestamp_to_seconds(subtitle["end"])
            
        start -= starting_timestamp_for_subtitles
        end -= starting_timestamp_for_subtitles
        subtitle_timestamp = (start + end) / 2
        subtitle_text = subtitle['text'] if "timestamp" in subtitle else subtitle["line"]
