...
class VideoSearcher:
    ...
    def sample_frames(self, num_samples: int, fps=1):
        ...
        return sampled_frame_indexes.tolist(), frames
    ...
    def search(self) -> Tuple[List[np.ndarray], List[float]]:
        ...
        while self.remaining_targets and self.search_budget > 0:
            ...
            # while self.remaining_targets and self.search_budget > 0:
            #     # 识别低置信度的帧，保留高置信度的25%
            #     num_high_conf = max(1, len(cell_detected_confidences) // 4)
            #     high_conf_indices = np.argsort(cell_detected_confidences)[-num_high_conf:]
            #     retained_frame_indices = [sampled_frame_indices[i] for i in high_conf_indices][:int(0.25*num_frames_in_grid)]
            #     retained_confidences = [cell_detected_confidences[i] for i in high_conf_indices]

            #     # 重新采样更多帧，集中在保留的区域
            #     additional_samples = int(num_frames_in_grid - len(retained_frame_indices))

            #     additional_frame_indices, additional_frames = self.sample_frames(additional_samples)
            ...
            #     sampled_frame_indices, frames = self.sample_frames(additional_samples)
            ...
            #     cell_detected_confidences += additional_frame_confidences
...
if __name__ == "__main__":
    ...
    target_objects = ["couch", "chair"]  # 例如，目标对象
